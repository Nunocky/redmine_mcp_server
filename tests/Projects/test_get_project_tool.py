...
def test_get_project_by_id_real_api():
    ...
    if not redmine_url or not api_key or not project_id:
        pytest.skip("REDMINE_URL, REDMINE_ADMIN_API_KEY, REDMINE_PROJECT_IDが未設定のためスキップ")
...
def test_get_project_by_identifier_real_api():
    ...
    print(f"REDMINE_URL={redmine_url}, REDMINE_ADMIN_API_KEY={'***' if api_key else ''}, REDMINE_PROJECT_IDENTIFIER={project_identifier}")
    if not redmine_url or not api_key or not project_identifier:
        pytest.skip("REDMINE_URL, REDMINE_ADMIN_API_KEY, REDMINE_PROJECT_IDENTIFIERが未設定のためスキップ")
